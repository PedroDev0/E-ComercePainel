...
public class ProdutoMapper {
	...
	public static ProdutoDTO toDTO(Produto produto) {
		...
		return new ProdutoDTO(produto.getId(), produto.getDescricao(), produto.getPrecoCompra(),
				produto.getPrecoVenda(), produto.getDataCadastro(),
				produto.getImagens().stream().map(ProdutoImagemMapper::toDTO).collect(Collectors.toList()));
	}
	...
	public static Produto toEntity(ProdutoDTO produtoDTO) {
		...
		produto.setImagens(produtoDTO.imagens().stream().map(imagemDTO -> {
			ProdutoImagem imagem = ProdutoImagemMapper.toEntity(imagemDTO);
